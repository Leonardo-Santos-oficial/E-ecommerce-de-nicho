  ...
  return (
    <section className="mt-10 rounded-lg border border-slate-800 p-4">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {items.map((i) => (
          <label key={i.id} className="flex items-center gap-3 p-2 rounded hover:bg-slate-800/40 cursor-pointer">
            <input
              type="checkbox"
            ...
            <div className="relative w-12 h-12 rounded overflow-hidden bg-slate-800">
              <Image src={i.image || '/placeholder.svg'} alt={i.name} fill className="object-cover" />
            </div>
            <div className="text-sm">
              <div className="text-slate-200">{i.name}</div>
      ...
      <div className="mt-4 flex items-center justify-between">
        <div className="text-slate-300">Total: <span className="text-cyan-400 font-semibold">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(total)}</span></div>
        <button className="btn btn-primary" onClick={() => onBuyTogether?.(Object.keys(selected).filter(k => selected[k]))}>Comprar Junto</button>
      </div>
    </section>
  )
}
